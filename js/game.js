...
function nextLevel() {
	LOCK = false;
 	GHOST_INKY_HAIRCUT=0;
	GHOST_BLINKY_HAIRCUT=0;
	GHOST_PINKY_HAIRCUT=0;
	GHOST_CLYDE_HAIRCUT=0;
	LEVEL ++;
...
function lifes(l) {
	...
	if (canvas.getContext) {
		...
		for (var i = 0, imax = LIFES; (i < imax && i < 4); i ++) {
			...
			const openScissors = document.getElementById("scissorsOpen");
			openScissors.addEventListener("load", (e) => {
			});
			// ctx.drawImage(Teddy, x-30, y-15, 30, 30);
			ctx.drawImage(openScissors, 13 + (i*33) + (i*1), 3, 30, 30);
			// ctx.beginPath();
			//
			// var lineToX = 13;
			// var lineToY = 15;
