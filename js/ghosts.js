...
var GHOST_CLYDE_HAIRCUT = 0;
var GHOST_BLINKY_HAIRCUT = 0;
var GHOST_PINKY_HAIRCUT = 0;
var GHOST_INKY_HAIRCUT = 0;
...
function gotHaircut(ghost) {
	switch(ghost.toUpperCase()){
		case "INKY":
			return GHOST_INKY_HAIRCUT
			break;
		case "PINKY":
			return GHOST_PINKY_HAIRCUT;
			break;
		case "BLINKY":
			return GHOST_BLINKY_HAIRCUT;
			break;
		case "CLYDE":
			return GHOST_CLYDE_HAIRCUT;
			break;
	}
}
function drawGhost(ghost) {
	...
	eval('drawHelperGhost(ctx, GHOST_' + ghost.toUpperCase() + '_POSITION_X, GHOST_' + ghost.toUpperCase() + '_POSITION_Y, GHOST_' + ghost.toUpperCase() + '_DIRECTION, GHOST_' + ghost.toUpperCase() + '_BODY_STATE, GHOST_' + ghost.toUpperCase() + '_STATE, GHOST_' + ghost.toUpperCase() + '_AFFRAID_STATE, gotHaircut(ghost))');
...
function eraseGhost(ghost) {
	...
	eval('ctx.clearRect(GHOST_' + ghost.toUpperCase() + '_POSITION_X - 17-6, GHOST_' + ghost.toUpperCase() + '_POSITION_Y - 17-6, 45, 45)');
}
...
// a = afraid , s = state, b = body state , d = diretion,
function drawHelperGhost(ctx, x, y, d, b, s, a, gotHaircut) {
	const Fro = document.getElementById("Fro");
	Fro.addEventListener("load", (e) => {
	});
	const LessFro = document.getElementById("LessFro");
	LessFro.addEventListener("load", (e) => {
	});
	if (s != -1) {
		ctx.beginPath();
	...
	if((s ===0 || s===1) && gotHaircut===0){
		ctx.drawImage(Fro, x-21, y-22, 40, 50);
	}
	else if((s ===0 || s===1) &&gotHaircut===1)
		ctx.drawImage(LessFro, x-21, y-22,40,40);
}
